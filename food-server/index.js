...
async function run() {
  try {
    ...
    const menuFoodCollection = client.db('foodService').collection('menuFood');
    const reviewsFoodCollection = client.db('foodService').collection('reviewsFood');
...
app.get('/', (req, res) => {
  res.send('Hello from Food Server..')
})
